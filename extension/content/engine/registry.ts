...
const ethnicity = /ethnicity|race|racial/i
const veteran_status = /veteran/i
const disability_status = /disability|disabled/i
const hispanic_latino = /hispanic|latin/i
...
const GROUP_ALL: Record<string, LeafRule> = {
  work_authorized, requires_sponsorship, gender, ethnicity, veteran_status, disability_status,
  hispanic_latino, transgender, would_relocate, non_compete, us_gov_employee, gov_contracting,
  export_restricted, f1_student, enrolled_returning, privacy_ack, degree_pursuing, grad_date,
}
...
const GROUP_BOOL: Record<string, LeafRule> = {
  ...
  gov_contracting, export_restricted, f1_student, enrolled_returning, privacy_ack,
}
// Enum-only subset (Workday mutually-exclusive checkbox groups).
const GROUP_ENUM: Record<string, LeafRule> = {
  gender, ethnicity, veteran_status, disability_status, hispanic_latino, transgender,
